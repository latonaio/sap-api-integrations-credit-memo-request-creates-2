...
import (
	...
	sap_api_output_formatter "sap-api-integrations-credit-memo-request-creates/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(header *requests.Header) {
	outputDataHeader, err := c.callCreditMemoRequestSrvAPIRequirementHeader("A_CreditMemoRequest", header)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callCreditMemoRequestSrvAPIRequirementHeader(api string, header *requests.Header) (*sap_api_output_formatter.Header, error) {
...
func (c *SAPAPICaller) Item(item *requests.Item) {
	url := fmt.Sprintf("A_CreditMemoRequest('%s')/to_Item", item.CreditMemoRequest)
	outputDataItem, err := c.callCreditMemoRequestSrvAPIRequirementItem(url, item)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callCreditMemoRequestSrvAPIRequirementItem(api string, item *requests.Item) (*sap_api_output_formatter.Item, error) {
